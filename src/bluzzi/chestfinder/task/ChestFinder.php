            ...
            for($z = $this->player->getPosition()->getZ() - $radius; $z <= $zMax; $z += 16){
                if(!$this->player->getPosition()->getWorld()->isChunkLoaded($x >> 4, $z >> 4)){
                    ...
                    foreach($chunk->getTiles() as $tile){
                        if(!in_array(
                            $tile->getBlock()->getId() == VanillaBlocks::TRAPPED_CHEST()->getId() ? "TrappedChest" : $tile::class, 
                            Main::$detects)
                        ) continue;

                        if(	
                            $this->player->getPosition()->distance($tile->getBlock()->getPosition()->asVector3()) 
                            <= Main::getDefaultConfig()->get("radius")
                        ){
                            if(empty($theChest)){
                                $theChest = $tile;
                            } else {
                                if(
                                    $this->player->getPosition()->distance($tile->getBlock()->getPosition()->asVector3()) 
                                    < $this->player->getPosition()->distance($theChest->getPosition()->asVector3())
                                ) $theChest = $tile;
                            }
